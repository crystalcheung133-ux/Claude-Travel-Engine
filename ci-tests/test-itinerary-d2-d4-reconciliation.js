...
assert.equal(BOOKINGS_DATA['bk-pizza4ps'].day,2);assert.equal(BOOKINGS_DATA['bk-pizza4ps'].time,'13:00');assert.equal(BOOKINGS_DATA['bk-lune'].time,'18:30');assert.equal(BOOKINGS_DATA['bk-moc-huong'].day,4);assert.equal(BOOKINGS_DATA['bk-little-bear'].day,4);
const ids=d=>ITINERARY_DATA[String(d)].items.map(x=>x.id);
assert.equal(JSON.stringify(ids(2)),JSON.stringify(['com-tam-moc','garmentory','shopping-tqd','pizza4ps','shopping-nguyen-trai','kocochi','lune']));
assert.equal(JSON.stringify(ids(3)),JSON.stringify(['cu-chi','man-moi','war-museum','nara-spa','oc-dao']));
assert.equal(JSON.stringify(ids(4)),JSON.stringify(['running-bean','pink-church','push-push','quan-thuy','thao-dien-walk-1','bakes','moc-huong','thao-dien-walk-2','little-bear']));
...
assert(ITINERARY_DATA['4'].items.find(x=>x.id==='quan-thuy').route.includes('一次 Grab'),'Day 4 one-Grab principle missing');
