...
assert.deepEqual(ids(2),['com-tam-moc','garmentory','shopping-tqd','pizza4ps','norah-spa-2','vincom-new-playground','post-office','lune']);
for(const [id,time,event] of [['bk-pizza4ps','12:45','pizza4ps'],['bk-norah-spa-2','14:00','norah-spa-2'],['bk-lune','18:45','lune']]){assert(B[id]);assert.equal(B[id].day,2);assert.equal(B[id].date,'2026-10-31');assert.equal(B[id].time,time);assert.equal(B[id].timelineItemId,event)}
