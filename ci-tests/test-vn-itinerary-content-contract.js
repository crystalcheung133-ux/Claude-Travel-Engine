...
assert.equal(I['2'].heading,'Slow Lifestyle Day');assert.equal(I['4'].heading,'Fashion Day');
for(const [id,day,event,emoji] of [['bk-moc-huong',2,'moc-huong','🌿'],['bk-moc-healing',4,'moc-healing','🦶'],['bk-nara',3,'nara-spa','🫧']]){assert.equal(B[id].day,day);assert.equal(B[id].timelineItemId,event);assert.equal(B[id].emoji,emoji);assert(B[id].notes.length>45);}
assert(B['bk-man-moi']&&B['bk-man-moi'].day===3&&B['bk-man-moi'].timelineItemId==='man-moi');
assert(I['4'].items.find(x=>x.id==='pizza4ps').route.includes('Mộc Healing'));
assert(I['2'].items.find(x=>x.id==='quan-thuy').route.includes('OHQUAO'));
