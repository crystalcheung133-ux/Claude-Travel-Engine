...
(async()=>{
 ...
 try{
  ...
  const identityModal=page.locator('#mamaModal');assert(await identityModal.evaluate(el=>el.classList.contains('show')),'First device launch must require traveller selection');assert.equal(await page.locator('#mamaModal .mama-close').evaluate(el=>getComputedStyle(el).display),'none','Required traveller selector must not be dismissible');await page.locator('#mamaModal .family-choice[data-family="crystal"]').click();await page.waitForTimeout(20);assert(!(await identityModal.evaluate(el=>el.classList.contains('show'))),'Traveller selector did not close after selection');assert(/Crystal/.test(await page.locator('.friend-pill').innerText()),'Header identity pill must show selected traveller name');await page.locator('.friend-pill').click();await page.waitForTimeout(10);assert(await identityModal.evaluate(el=>el.classList.contains('show')),'Top-right traveller identity button is not clickable');await page.locator('#mamaModal .mama-close').click();
  ...
  const first=page.locator('.timeline-item').first();if(await first.count()){const time=await first.locator('.timeline-time').boundingBox();assert(time&&time.width<=72,'Mobile timeline time rail is too wide');}const routeHints=page.locator('.route-hint');for(let i=0;i<await routeHints.count();i++){const txt=await routeHints.nth(i).innerText();assert((txt.match(/To next stop/gi)||[]).length<=1,'Timeline route repeats To next stop label');}
  ...
  const tripMenuText=await page.locator('#tripMenu').innerText();const vnBuild=String(await page.evaluate(()=>TRIP_CONFIG&&TRIP_CONFIG.id||'')).includes('vietnam'); if(vnBuild){assert(/Activities & Transport/.test(tripMenuText),'VN sparse Activity/Transport modules are not grouped');assert(!/^Activities$/m.test(tripMenuText)&&!/^Transport$/m.test(tripMenuText),'VN grouped modules still render duplicate standalone menu entries');}else{assert(/Stay/.test(tripMenuText),'Trip menu missing Stay');}assert(!/^Bookings$/m.test(tripMenuText),'Consolidated Bookings entry must not appear in Trip menu');
