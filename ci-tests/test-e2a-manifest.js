...
// --- Current Trip fixture: generate from the real repo root ---
{
  const cfg = loadConfig(REPO_ROOT);
  const manifest = buildManifest(cfg);
  if (manifest.name === cfg.TRIP_CONFIG.tripName) ok('current fixture: manifest.name matches TRIP_CONFIG.tripName');
  else fail('current fixture: manifest.name was "' + manifest.name + '"');
  if (manifest.short_name === cfg.TRIP_CONFIG.shortName) ok('current fixture: manifest.short_name matches TRIP_CONFIG.shortName');
  else fail('current fixture: manifest.short_name was "' + manifest.short_name + '"');
  if (manifest.background_color === cfg.THEME_CONFIG.colors.background && manifest.theme_color === cfg.THEME_CONFIG.colors.background)
    ok('current fixture: manifest colours match THEME_CONFIG.colors.background');
  else fail('current fixture: manifest colours do not match THEME_CONFIG');
  for (const icon of manifest.icons) {
    if (fs.existsSync(path.join(REPO_ROOT, icon.src))) ok(`current fixture: icon file exists on disk: ${icon.src}`);
    else fail(`current fixture: icon file referenced by manifest does not exist: ${icon.src}`);
  }
}
...
// --- non-NZ fixture: synthetic fixture directory with different config ---
{
  ...
  fs.copyFileSync(path.join(REPO_ROOT, 'assets/japan-onsen-logo.png'), path.join(fixtureDir, 'assets/japan-onsen-logo.png'));
