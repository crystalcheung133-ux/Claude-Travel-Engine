...
function friendIdentityHTML(key,compact=false){
  ...
  return `<span class="family-identity family-${escapeHTML(key)}${compact?' is-compact':''}"><span class="family-code">${escapeHTML(identity.emoji||identity.code)}</span><span class="family-name">${escapeHTML(identity.name)}</span></span>`;
}
